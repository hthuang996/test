...
mod erc20 {
    ...
    impl Erc20 {
        ...
        #[ink(message)]
        pub fn mint(&mut self, account: AccountId, value: Balance) -> Result<()> {
            let caller = Self::env().caller();
            if caller != self.owner {
                return Err(Error::NotOwner);
            }

            self.mint_impl(&account, value);
            Ok(())
        }

        fn mint_impl(&mut self, account: &AccountId, value: Balance) {
        ...
        #[ink(message)]
        pub fn burn(&mut self, account: AccountId, value: Balance) -> Result<()> {
            let caller = Self::env().caller();
            if caller != self.owner {
                return Err(Error::NotOwner);
            }

            self.burn_impl(&account, value)?;
            Ok(())
        }

        fn burn_impl(&mut self, account: &AccountId, value: Balance) -> Result<()> {
            let balance = self.balances.get(&account).unwrap_or_default();
            if balance < value {
                return Err(Error::InsufficientBalance);
            }
    ...
    mod tests {
        ...
        fn set_caller(sender: AccountId) {
            env::test::set_caller::<env::DefaultEnvironment>(sender);
        }
        ...
        fn new_works() {
            // Constructor works.
            let _erc20 = Erc20::new("NAME".to_string(), "SYMBOL".to_string());
        }
        ...
        fn total_supply_works() {
            // Constructor works.
            let erc20 = Erc20::new("NAME".to_string(), "SYMBOL".to_string());
        ...
        fn mint_works() {
            // Constructor works.
            let mut erc20 = Erc20::new("NAME".to_string(), "SYMBOL".to_string());
            let accounts =
                ink::env::test::default_accounts::<ink::env::DefaultEnvironment>();
            set_caller(accounts.bob);
            assert_eq!(
                erc20.mint(accounts.alice, 100),
                Err(Error::NotOwner)
            );
            set_caller(accounts.alice);
            // Alice mint 100 tokens to Bob.
